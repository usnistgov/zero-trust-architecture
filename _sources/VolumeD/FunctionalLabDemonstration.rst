...
As defined in NIST SP 800-63, an *identity* is an attribute or set of attributes that uniquely identifies a subject. Here, a *network identity* is used simply as an identity that allows the subject to identify itself to all (network) connected enterprise resources. The following definitions are used for network IDs:
